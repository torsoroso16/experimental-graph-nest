...
export class CouponsResolver {
  ...
  verifyCoupon(
    @Args('input') verifyCouponInput: VerifyCouponInput,
  ): VerifyCouponResponse {
    return this.couponsService.verifyCoupon(verifyCouponInput);
  }

  @Mutation(() => Coupon)
  approveCoupon(@Args('id', { type: () => ID }) id: number) {
    return this.couponsService.approveCoupon(id);
  }
  @Mutation(() => Coupon)
  disapproveCoupon(@Args('id', { type: () => ID }) id: number) {

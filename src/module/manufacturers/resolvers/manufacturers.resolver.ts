...
import { GetTopManufacturerArgs } from 'src/module/common/dto/get-top-manufacturer.args';
...
export class ManufacturersResolver {
  ...
  async topManufacturers(
    @Args() getTopManufacturerArgs: GetTopManufacturerArgs,
  ): Promise<Manufacturer[]> {
    return this.manufacturersService.topManufacturers(
      getTopManufacturerArgs.limit,
    );
  }
}